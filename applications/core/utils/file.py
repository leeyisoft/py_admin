...
class Uploader():
    @staticmethod
    def upload_img(img):
        path = settings.ROOT_PATH + '/upload/'
        path = path.replace('//', '/')
        ...
        path_file = path + user_id + '.' + file_ext

        # print('filename', type(filename), dir(filename), path_file)
        ...
        param = {
            ...
            'origin_name': FileUtil.file_name(path_file),
            'path_file': img['filename'],
            'user_id': user_id,
