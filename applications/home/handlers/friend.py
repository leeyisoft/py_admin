...
class FindHandler(CommonHandler):
    ...
    def _find_friend(self, value, page, limit):
        ...
        if items:
            for row in items:
                item = row.as_dict(['uuid', 'username', 'avatar', 'sign'])
...
class ApplyAddFriendHandler(CommonHandler):
    ...
    def post(self, *args, **kwargs):
        ...
        # end if
        MemberFriend.Q.filter(MemberFriend.uuid==friend.uuid).update(params)

        # for notice
        query = MemberFriendNotice.Q.filter(MemberFriendNotice.status==0)
        query = query.filter(MemberFriendNotice.related_uuid==friend.uuid)
        notice = query.first()
        params2 = {
            'msgtype': 'apply_friend',
            'related_uuid': friend.uuid,
            'message': remark,
            'from_user_id': user_id,
            'to_user_id': to_user_id,
            'status': 0,
        ...
        if notice is None:
            params2['uuid'] = Func.uuid32()
            notice = MemberFriendNotice(**params2)
            MemberFriendNotice.session.add(notice)
        else:
            MemberFriendNotice.Q.filter(MemberFriendNotice.uuid==notice.uuid).update(params2)
...
class AddFriendHandler(CommonHandler):
    ...
    def post(self, *args, **kwargs):
        ...
        query = MemberFriend.Q.filter(MemberFriend.uuid==friend_id)
        friend = query.first()
        if friend is None:
        ...
        MemberFriend.Q.filter(MemberFriend.uuid==friend_id).update(params)
        ...
        if action=='agree':
            ...
            if to_friend:
                ...
                MemberFriend.Q.filter(MemberFriend.uuid==to_friend.uuid).update(params)
            else:
                params = {
                    'uuid': Func.uuid32(),
        ...
        query = query.filter(MemberFriendNotice.related_uuid==friend_id)
        notice = query.first()
